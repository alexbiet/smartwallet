...
window.addEventListener("load", async () => {
  ...
  document
    .querySelector("#radio-rinkeby")
    .addEventListener("click", function () {
      switchNetworkRinkeby();
    });
  document
    .querySelector("#radio-mumbai")
    .addEventListener("click", function () {
      switchNetworkMumbai();
    });
  document
    .querySelector("#radio-polygon")
    .addEventListener("click", function () {
...
async function fetchAccountData() {
  ...
  if (!useRPC) {
    ...
    network = chainIdMap[Number(ethereum.chainId)].name
      .split(" ")[0]
      .toLowerCase();
  ...
  async function generateCards(_tokenArray) {
    ...
    !useRPC
      ...
      : (nativePrice = "$1,500.00");
    !useRPC
      ? (depositedValue = await getDepositedValue("ETH"))
      : (depositedValue = 0);
    !useRPC ? (nativeRate = await getRates("ETH")) : (nativeRate = 0);
    ...
    document.getElementById(`deposited-${nativeAsset}`).innerHTML =
      depositedValue;
    document.getElementById(`interest-${nativeAsset}`).innerHTML =
      nativeRate + "%";

    if (network === "polygon") {
      document
        .querySelector(`#btn-buy${nativeAsset}`)
        .addEventListener("click", function () {
          launchTransak(account, nativeAsset);
        });
    } else if (network === "rinkeby") {
      document
        .querySelector(`#btn-faucet${nativeAsset}`)
        .addEventListener("click", function () {
    ...
    document.getElementById(`btn-withdraw${nativeAsset}`).onclick =
      function () {
        let amountValue = document.getElementById(
          `amount-${nativeAsset}`
        ).value;
    ...
    for (let i = 0; i < _tokenArray.length; i++) {
      ...
      if (network === "polygon") {
        document
          .querySelector(`#btn-buy${tokenId}`)
          .addEventListener("click", function () {
            launchTransak(account, tokenId);
          });
      } else {
  ...
  async function getPrice(_token) {
    ...
    if (assetName === "MATIC" || assetName === "ETH") return price;
    else {
  ...
  async function getDepositedValue(_token) {
    ...
    if (_token === "ETH") return returnVal / 10 ** decimals;
    else {
      document.getElementById(`deposited-${_token}`).innerHTML =
        returnVal / 10 ** decimals;
    }
  }
  ...
  async function getRates(_token) {
    ...
    let depositAPY =
      ((1 + depositAPR / 31536000) ** 31536000 - 1).toFixed(4) * 100; //secondsPerYearHardcoded
    if (_token === "ETH") {
      return depositAPY.toFixed(2);
    } else {
      document.getElementById(
        `interest-${_token}`
      ).innerHTML = `${depositAPY.toFixed(2)}%`;
    }
  }
  ...
  async function supplyERC20(_token, _amount) {
    ...
    const transaction = await contractCall.supply(
      tokenAddress,
      amount,
      account,
      0
  ...
  async function withdrawERC20(_token, _amount) {
    ...
    const transaction = await contractCall.withdraw(
      tokenAddress,
      amount,
      account
    );
  ...
  async function supplyETH(_amount) {
    ...
    const transaction = await contractCall.depositETH(
      await getPoolContractAddress(),
      account,
      0,
      { value: amount }
  ...
  async function withdrawETH(_amount) {
    ...
    // pool, amount, to
    const transaction = await contractCall.withdrawETH(
      await getPoolContractAddress(),
      amount.toString(),
      account
    );
          ...
          params: [
            {
              ...
              chainName: "Mumbai",
              rpcUrls: [
                "https://polygon-mumbai.g.alchemy.com/v2/SqfaXTHKq6BcbY6g2ZaQ2OtxwjbsW1Wk",
              ],
              nativeCurrency: {
                name: "Matic",
                symbol: "Matic",
...
var popover1 = new bootstrap.Popover(
  document.querySelector(".popover-dismiss-1"),
  {
    trigger: "focus",
    html: true,
    // title: 'Title 1',
